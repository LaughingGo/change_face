...
from model_loss import diff_loss, recon_loss
...
if __name__ == '__main__':
    ...
    eval_index_list = pair_list[40000:50000]
    ...
    ###############################################################################
    encoder = Encoder()
    decoder = Decoder(shortcut_layer=3)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            loss_recon_cur = recon_loss(img_1_trans, img_1) +  recon_loss(img_2_trans, img_2) 
            
            loss_cur = loss_z_cur + args.alpha * loss_recon_cur
            ...
            ' | time {batch_time:.3f} | loss {loss.val:.5f} | loss_z {loss_z.val:.5f} | loss_recon {loss_recon.val:.5f} |'.format(
                batch_time=batch_time, loss=loss, loss_z=loss_z, loss_recon=loss_recon), end_string="")
             
        with open(os.path.join(args.save, 'train.log'), 'a') as f:
                    f.write('epoch {}:\n'.format(epoch))
                    log_entry =   ' | time {:.3f} | loss {:.5f} | loss_z {:.5f} | loss_recon {:.5f} | \n'.format(
                        time.time()-epoch_start_time, loss.avg, loss_z.avg, loss_recon.avg)
                    f.write(log_entry)
     ...
     #logger.log_scalar('train_loss',train_loss, epoch)
        ...
        for batch_idx, data in enumerate(val_loader):
            ...
            psnr_cur = psnr(img_1_trans, img_1) + psnr(img_2_trans, img_2)
            ...
            ' | time {:.3f} | loss_val {:.5f} | loss_z_val {:.5f} | loss_recon_val {:.5f} | '.format(
              batch_time, loss_val.val, loss_z_val.val, loss_recon_val.val), end_string="")
                
        log_entry_val = '\n| end of epoch {:3d} | time: {:5.5f}s | valid loss {:.5f} | valid recon loss {:.5f} | valid psnr {:5.2f}'.format(
            epoch, (time.time() - epoch_start_time),loss_val.avg, loss_recon_val.avg, psnr_val.avg)
        print(log_entry_val)
